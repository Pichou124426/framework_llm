...
Architecture : les embeddings (Retriever/Indexeur) passent par Ollama en local ;
la génération (Generation/Post_generation) passe par Azure OpenAI, configuré via
un fichier `.env` (voir README).
...
def render_attack_result(result: dict):
    ...
    for i, step in enumerate(steps, start=1):
        with st.expander(f"Étape {i} — {step.get('title', '')}", expanded=(i == len(steps))):
...
def render_rag_page():
    ...
    st.caption("Embeddings calculés via Ollama en local.")
    ...
    if check_azure_configured():
        st.caption(f"Génération via Azure OpenAI — déploiement configuré : `{AZURE_DEPLOYMENT}`")
    else:
        manquantes = [k for k, present in azure_config_status().items() if not present]
        st.warning(
            "⚠️ Configuration Azure OpenAI incomplète. Variable(s) manquante(s) dans le fichier `.env` : "
            + ", ".join(f"`{m}`" for m in manquantes)
            + ". La construction de la cible et les attaques échoueront tant que ces variables ne sont pas renseignées."
        )
    ...
    if st.button("🏗️ Construire la cible RAG", type="primary", disabled=build_disabled):
        try:
            ...
            generation_config = GenerationConfig(
                ai_model=AZURE_DEPLOYMENT or "azure-deployment",
            ...
            st.error(f"Impossible de construire la cible : {exc}")
        except Exception as exc:
            st.error(f"Échec de la construction de la cible : {exc}")
            with st.expander("Détails techniques"):
                st.code(traceback.format_exc())
    ...
    if st.session_state.rag_config:
        ...
        with st.expander("📋 Résumé de la cible construite", expanded=False):
            ...
            st.json({
                ...
                "generation": {
                    "deploiement_azure": cfg.generation_config.ai_model,
...
def render_about_page():
    ...
    st.markdown(
        """
Cette interface met à disposition de personnes non expertes du RAG le framework offensif
...
- Ollama (`localhost:11434`) et ChromaDB (`localhost:8000`) démarrés — voir `docker-compose.yml`
  (`docker compose up -d`).
- Un fichier `.env` à la racine du projet renseignant `AZURE_OPENAI_API_ENDPOINT`,
  `AZURE_OPENAI_API_KEY`, `AZURE_OPENAI_API_VERSION` et `AZURE_OPENAI_DEPLOYMENT_NAME`.
        """
    )
...
with st.sidebar:
    ...
    azure_ok = check_azure_configured()
    st.markdown(f"{'🟢' if ollama_ok else '🔴'} Ollama — embeddings (`localhost:11434`)")
    st.markdown(f"{'🟢' if chroma_ok else '🔴'} ChromaDB (`localhost:8000`)")
    st.markdown(f"{'🟢' if azure_ok else '🔴'} Azure OpenAI — génération (config `.env`)")
    if not (ollama_ok and chroma_ok):
        st.caption("Démarrez les services avec `docker compose up -d` si besoin.")
    if not azure_ok:
        st.caption("Renseignez le fichier `.env` (voir README) pour activer la génération.")
